        ...
        <div className="max-w-md">
            ...
            <form onSubmit={submit} className="space-y-3 bg-white border rounded-2xl p-5">
                <div>
                    ...
                    <input className="w-full border rounded-xl p-2" value={name} onChange={(e) => setName(e.target.value)} required/>
                </div>
                <div>
                ...
                <p className="text-sm text-gray-600">¿Ya tienes cuenta?{" "}<Link to="/login" className="underline">Ingresar</Link></p>
            </form>
        </div>
    );
}
